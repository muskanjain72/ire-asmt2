...
\begin{enumerate}[leftmargin=1.8em, itemsep=2pt]
    ...
    \item \textbf{Old GBDT Baseline (AUC 0.5703, $N=500$)}: Pre-fix re-ranker evaluated in the early training run where session features were unconditionally zero due to unjoined impression history.
    \item \textbf{Canonical GBDT (AUC 0.6494, $N=3,000$)}: Loaded from checkpoint \nolinkurl{models/ebnerd_reranker.joblib} (trained on 10,000 chronological impressions with 50.54\% non-zero session features). Soundly outperforms official NRMS by $+0.0687$ AUC ($p=0.001$) and Stage-1 baseline by $+0.1560$ AUC ($p=0.001$).
...
-- Category Affinity & -0.0182 & -0.0050 & -0.0046 & -0.0063 & 0.0000 & 0.0000 & 0.0000 & 0.0000 \\
-- Freshness \& Recency & -0.0004 & -0.0007 & -0.0009 & -0.0006 & -0.1030 & -0.0662 & -0.0832 & -0.0680 \\
-- Popularity Prior & -0.0143 & -0.0133 & -0.0112 & -0.0105 & +0.0084 & +0.0084 & +0.0093 & +0.0068 \\
-- History Embeddings & -0.0118 & -0.0026 & -0.0013 & -0.0021 & 0.0000 & 0.0000 & 0.0000 & 0.0000 \\
\midrule
...
\noindent At standard operating size $K = 100$, the system delivers a p99 latency of \textbf{11.07 ms} (MIND) and \textbf{8.11 ms} (EB-NeRD), offering over \textbf{$9\times$ safety headroom} beneath the 100 ms target SLA.
...
\begin{itemize}[itemsep=1pt]
    ...
    \item \textbf{Daily Operating Cost}: Serving 10 million recommendations daily costs approximately \textbf{\$0.70 per day} (\$21/month).
\end{itemize}
