...
from src.recommenders_mind.iterator import BaseIterator
from src.recommenders_mind.newsrec_utils import word_tokenize, newsample
...
class MINDIterator(BaseIterator):
    ...
    def init_behaviors(self, behaviors_file, max_behaviors=None):
        """init behavior logs given behaviors file.

        Args:
            behaviors_file: path of behaviors file
            max_behaviors (int, optional): maximum behaviors to load.
        """
        self.histories = []
        self.imprs = []
        self.labels = []
        ...
        with tf.io.gfile.GFile(behaviors_file, "r") as rd:
            impr_index = 0
            for line in rd:
                if max_behaviors is not None and impr_index >= max_behaviors:
                    break
                parts = line.strip("\n").split(self.col_spliter)
                if len(parts) < 4:
                    continue
                uid, time, history, impr = parts[-4:]

                history_ids = [self.nid2index[i] for i in history.split() if i in self.nid2index]
                history = [0] * (self.his_size - len(history_ids)) + history_ids[
                    -self.his_size :
                ]

                impr_tokens = [i.split("-") for i in impr.split() if "-" in i]
                impr_news = [self.nid2index[tok[0]] for tok in impr_tokens if tok[0] in self.nid2index]
                label = [int(tok[1]) for tok in impr_tokens if tok[0] in self.nid2index]
                if not impr_news:
                    continue
                uindex = self.uid2index.get(uid, 0)
    ...
    def load_data_from_file(self, news_file, behavior_file, max_behaviors=None):
        ...
        if not hasattr(self, "impr_indexes"):
            self.init_behaviors(behavior_file, max_behaviors=max_behaviors)
    ...
    def load_user_from_file(self, news_file, behavior_file, max_behaviors=None):
    ...
    def load_impression_from_file(self, behaviors_file, max_behaviors=None):
        """Read and parse impression data from behaviors file.

        Args:
            behaviors_file (str): A file contains several informations of behaviors.
            max_behaviors (int, optional): maximum behaviors to load.
        ...
        if not hasattr(self, "histories"):
            self.init_behaviors(behaviors_file, max_behaviors=max_behaviors)
