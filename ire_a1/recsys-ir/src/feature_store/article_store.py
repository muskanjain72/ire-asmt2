...
class ArticleFeatureStore:
    ...
    def get_article_metadata(self, article_id: str) -> dict[str, Any] | None:
        """Return category, entities, published_at, embedding_ref for an article."""
        return self._store.get_by_id(
            "article_id",
            article_id,
            columns=["article_id", "category", "subcategory", "entities", "published_at", "embedding_ref"],
        )

    def get_article_freshness(
        self, article_id: str, as_of_ts: datetime
    ) -> float | None:
        """Return age of article in hours at as_of_ts (as_of_ts - published_at).

        Returns None if article not found or published_at is None (e.g. MIND).
        Raises ValueError if published_at is strictly after as_of_ts + tolerance (leakage prevention).
        """
        ...
        # Allow 24-hour buffer for timezone skew / embargo differences consistent with leakage_test
        delta_seconds = (as_of_ts - pub_dt).total_seconds()
        if delta_seconds < -86400:
            raise ValueError(
                f"Data leakage detected: article {article_id} published at {pub_dt} "
    ...
    def build_features(
        ...
        if "published_at" in df.columns:
            feature_cols.append("published_at")
        else:
            df = df.with_columns(pl.lit(None, dtype=pl.Datetime("us")).alias("published_at"))
            feature_cols.append("published_at")
